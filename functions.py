...
def load_tasks(header, tasks):
    ...
    with left_column:
            show_hint = st.toggle("nápověda", value=st.session_state.toggle_state, key="hint_toggle_state")
    with middle_column:
        show_solution = st.toggle("řešení", value=st.session_state.toggle_state, key="solution_toggle_state")
    with right_column:
        show_demonstration = st.toggle("ukázka", value=st.session_state.toggle_state, key="demonstration_toggle_state")
    st.divider()
